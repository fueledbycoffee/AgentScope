...
function errorMessage(error: unknown): string {
  ...
  return error instanceof ApiError && error.code === 'payload_too_large'
    ? `${message} The upload limit is 25 MiB; choose a smaller trace file.`
    : message
}
...
export default function ImportPage() {
  ...
  function PreviewStop() {
    ...
    const isBatch = state.entries.length > 1
    return <>
      <StageHeader title="Dry run on up to 200 records per file">{allPreviewed
      ...
      {!busy && !previewError && allPreviewed && <>
        ...
        <section className="import-route-stack"><h2>Rejects in the sample</h2>
          {aggregate.rejects.length > 0 ? <Table caption="Rejects sample" headers={[...(isBatch ? ['File'] : []), 'Locator', 'Rule', 'Path', 'Code', 'Field', 'Message']}>
            {aggregate.rejects.map((row, index) => <tr key={`${row.source.sha256}-${row.locator}-${index}`}>
              {isBatch && <td>{row.source.filename}<br /><span className="hash">{row.source.sha256.slice(0, 12)}…</span></td>}
              <td>{row.locator}</td><td>{row.rule_id}</td><td>{row.path}</td><td>{row.code}</td><td>{row.field ?? '—'}</td><td>{row.message}</td>
            </tr>)}
        ...
        <section className="import-route-stack"><div className="import-route-section-title"><h2>Emissions sample</h2><span>first five across the ordered files</span></div>
          <Table caption="Emissions sample" headers={[...(isBatch ? ['File'] : []), 'Entity', 'Path', 'Locator', 'Fields']}>
            {aggregate.emissions.map((row, index) => <tr key={`${row.source.sha256}-${row.locator}-${index}`}>
              {isBatch && <td>{row.source.filename}<br /><span className="hash">{row.source.sha256.slice(0, 12)}…</span></td>}
              <td>{row.entity}</td><td>{row.path}</td><td>{row.locator}</td><td><JsonView value={row.fields} /></td>
            </tr>)}
          </Table>
        </section>
      </>}
      <StageBar onBack={() => goTo(2)} status={busy === 'preview' ? 'Dry run in progress · nothing written' : undefined}
        primary={previewError
          ...
          : !allPreviewed
            ? <button type="button" className="btn primary" disabled={busy !== null} onClick={() => void runPreview()}>Run the dry run</button>
            : <button type="button" className="btn primary" disabled={busy !== null} onClick={() => goTo(4)}>Looks right, continue</button>} />
    </>
  }
