...
import { Counts, ErrorNotice, JsonView, Pagination, ResourceState, Table } from '../components'
...
export function ImportsPage() {
  ...
  return <><h1>Imports history</h1><ResourceState {...resource} />
    {resource.data && <>
      <Table caption="Import attempts, newest first" headers={['Import', 'Source', 'Status', 'Started', 'Files', 'Records', 'Entities']}>
        {resource.data.map(report => <tr key={report.import_id}>
          ...
          <td>{Object.entries(report.entities).map(([key, value]) => `${key}: ${value}`).join(', ')}</td>
        </tr>)}
      </Table>
  ...
  return <><h1>Import report</h1><ResourceState {...resource} />
    {report && <>
      ...
      {report.status === 'failed' && <div role="alert"><p>Import failed. No observations were inserted.</p>
        {report.error && <><ErrorNotice error={new Error(report.error.message)} /><JsonView value={report.error} /></>}</div>}
      <p>Records count source records; entities count emitted observations.</p>
      <Counts title="Source records" counts={report.records} />
      <Counts title="Entity observations" counts={report.entities} />
