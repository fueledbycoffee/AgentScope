...
URL = ("https://github.com/uw-syfi/TraceLab/releases/download/v0.0.1/"
...
def ensure_source(path):
    ...
    if not path.exists():
        ...
        with tempfile.NamedTemporaryFile(dir=path.parent, delete=False) as stream:
            ...
            try:
                ...
                raise
        temporary.replace(path)
        write_json(receipt_path, {"url": URL, "sha256": UPSTREAM_SHA256,
                                  "bytes": UPSTREAM_BYTES, "retrieved_at": utc_now()})
    ...
    if receipt_path.exists():
        ...
        if (receipt.get("sha256"), receipt.get("bytes"), receipt.get("url")) != (
                digest, size, URL):
            raise ValueError("Retrieval receipt does not match release")
        retrieved_at = receipt.get("retrieved_at")
    return {"url": URL, "version": "v0.0.1", "sha256": digest, "bytes": size,
            "retrieved_at": retrieved_at,
            "retrieval_note": ("Recorded after verified download" if retrieved_at else
                               "Unknown: pre-existing cache has no retrieval receipt")}
    ...
    with gzip.open(source, "rb") as stream:
        for ordinal, line in enumerate(stream, 1):
            try:
                ...
                if any(x is not None and not isinstance(x, str)
                       for x in (project, session_file)):
                    raise ValueError("invalid session metadata")
            except (ValueError, KeyError, TypeError) as exc:
                raise ValueError("Invalid source record at line %d" % ordinal) from exc
            info = sessions.setdefault(session_id, {
                "provider": provider, "row_count": 0, "source_files": set()})
            if info["provider"] != provider:
                raise ValueError("Session identity crosses provider strata")
            info["row_count"] += 1
            info["source_files"].add((project, session_file, "project" in row,
                                      "session_file" in row))
            rows[provider] += 1
...
def select_sessions(sessions, seed, count):
    ...
    for provider in PROVIDERS:
        ...
        if len(candidates) < count:
            raise ValueError("Requested %d %s sessions; only %d available" %
                             (count, provider, len(candidates)))

        def rank(sid):
            payload = json.dumps([seed, provider, sid], ensure_ascii=True,
    ...
    with tempfile.NamedTemporaryFile(dir=output.parent, delete=False) as raw:
        ...
        try:
            with gzip.GzipFile(filename="", mode="wb", fileobj=raw,
                               compresslevel=9, mtime=0) as target:
                with gzip.open(source, "rb") as stream:
                    for ordinal, line in enumerate(stream, 1):
    ...
    for provider, ids in selected.items():
        for sid in ids:
            ...
            selected_details.append({
                "session_id": sid, "provider": provider, "row_count": counts[sid],
                "source_files": [{"project": p, "session_file": f,
                                  "project_present": has_project,
                                  "session_file_present": has_file}
                                 for p, f, has_project, has_file in
                                 sorted(info["source_files"], key=lambda pair: json.dumps(pair))],
                "source_line_ranges": locators[sid]})
    manifest = {
        "manifest_version": 1, "upstream": upstream, "generated_at": utc_now(),
        "selection": {"seed": seed, "sessions_per_provider": count,
                      "providers": list(PROVIDERS), "group_by": "session_id",
                      "algorithm": "sha256-json-array-rank-v1",
                      "rank_input": "UTF-8 JSON [seed,provider,session_id], ensure_ascii=true, compact separators",
        ...
        "source_counts": {"rows": sum(source_rows.values()),
                          "rows_per_provider": dict(source_rows),
                          "sessions_per_provider": dict(Counter(
                              info["provider"] for info in sessions.values()))},
        "selected_session_ids_per_provider": selected, "sessions": selected_details,
        "output": {"file": output.name, "sha256": digest, "bytes": size,
                   "uncompressed_bytes": uncompressed_bytes,
                   "rows": sum(counts.values()), "sessions": len(counts),
                   "rows_per_provider": {p: sum(counts[sid] for sid in ids)
                                         for p, ids in selected.items()},
                   "gzip": {"mtime": 0, "filename": "", "compresslevel": 9}},
    }
    write_json(manifest_path, manifest)
    print("Upstream: sha256=%s bytes=%d" % (upstream["sha256"], upstream["bytes"]))
    print("Upstream retrieved_at=%s" % upstream["retrieved_at"])
    print("Source: rows=%d sessions=%d" % (sum(source_rows.values()), len(sessions)))
    for provider, ids in selected.items():
        print("%s: available_sessions=%d selected_sessions=%d rows=%d" % (
            provider, manifest["source_counts"]["sessions_per_provider"][provider],
            len(ids), manifest["output"]["rows_per_provider"][provider]))
    print("Output: %s sha256=%s bytes=%d uncompressed_bytes=%d rows=%d" % (
        output, digest, size, uncompressed_bytes, sum(counts.values())))
...
def main():
    ...
    parser.add_argument("--manifest", type=Path,
                        help="Default: tracelab-sample.manifest.json next to output")
    ...
    protected = {args.source.resolve(), args.source.with_name(
        args.source.name + ".retrieval.json").resolve()}
    if output.resolve() in protected or manifest_path.resolve() in protected or (
            output.resolve() == manifest_path.resolve()):
        parser.error("source, receipt, output and manifest must have distinct paths")
    try:
        upstream = ensure_source(args.source)
        sample(args.source, output, manifest_path, upstream,
               args.seed, args.sessions_per_provider)
